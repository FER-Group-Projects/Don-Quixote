...
public class RandomInstructionUtility {
	...
	public static EasyLGPInstruction generateRandomLGPInstruction(long numOfRegisters, int programLength, long maxAbsMovConstant) {
		...
		switch(rnd) {
			...
			case 5 : return new AndInstruction(generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters));
			case 6 : return new OrInstruction(generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters));
			case 7 : return new XorInstruction(generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters));
			case 8 : return new CmpInstruction(generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters));
			case 9 : return new JpInstruction(generateRandomJpCondition(), random.nextInt(programLength));
			case 10 : return new HaltInstruction();
			case 11 : return new MovInstruction(generateRandomRegNumber(numOfRegisters), generateRandomRegNumber(numOfRegisters));
			case 12 : return new MovConstInstruction(generateRandomRegNumber(numOfRegisters), getRandomSign() * (long) (random.nextDouble() * maxAbsMovConstant));
